...
object VerificationExtractor {
  ...
  var cppHeader: Option[String] = None
}
...
class VerificationExtractor extends Phase {
  ...
  override def transform(annotations: AnnotationSeq): AnnotationSeq = {
    ...
    val newCircuit: AnnotationSeq = if (sinkAnnos.nonEmpty) {
    ...
    if (VerificationExtractor.cppHeader.isDefined) {
      val c_array = messages.map(msg => s"\"$msg\"").mkString(",\n")
      val c_str = Seq(
        "#ifndef ASSERTION_EXTRACTOR",
        s"#define ASSERTION_EXTRACTOR ${messages.length}",
        "static const char *assertion_messages[] = {",
        c_array,
        "};",
        "#endif",
      ).mkString("\n")
      Files.write(Paths.get(VerificationExtractor.cppHeader.get), c_str.getBytes(StandardCharsets.UTF_8))
    }
  ...
  private def onStmt(statement: Statement)(implicit tracker: AssertionTracker): (Seq[Statement], Statement) = {
    statement match {
      ...
      case v: Verification if v.op == Formal.Assert =>
        messages.append(v.info.toString) // TODO: v.msg is empty now. Should fix upstream Chisel.
...
private object DefRegisterWithRef {
  def apply(
    ...
    (DefRegisterWithReset(info, name, tpe, clock, reset, init), Reference(name, tpe))
  }
  ...
  def apply(
    ...
    (DefRegister(info, name, tpe, clock), Reference(name, tpe))
  }
}
